...
// Strip wiki disambiguation suffixes: "(movie)", "(film)", "(2026 film)",
// "(TV series)", etc. — the standard wiki convention for same-named articles.
const DISAMBIG = /\s*\([^)]*\)\s*$/
...
async function searchTitle(title: string): Promise<PresenceArticle[]> {
  ...
  const response = await fetch(url, { signal: AbortSignal.timeout(PRESENCE.timeoutMs) })
  ...
  for (const result of data.results) {
    ...
    const dedup = `${result.wikiId}:${normalise(result.title)}`
...
export async function attach(titles: Title[]): Promise<PresenceResult> {
  ...
  await pooled(titles, PRESENCE.concurrency, async (title) => {
    try {
